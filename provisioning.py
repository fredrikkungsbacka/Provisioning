...
def port_trunk(model: dict, crt_tab_object: object) -> None:
  ...
  if model["image"]["xe"] is True:
    crt_sendline("device-tracking attach-policy kba-trunk", crt_tab_object)
  else:
    if model["image"]["version"].startswith("15.0"):
      crt_sendline("ip device tracking maximum 1", crt_tab_object)
    else:
      crt_sendline("ip device tracking maximum 0", crt_tab_object)
    ...
    crt_sendline("nmsp attachment suppress", crt_tab_object)

  # generic config
  crt_sendline("description TRUNK", crt_tab_object)
  crt_sendline("switchport mode trunk", crt_tab_object)
  crt_sendline("storm-control broadcast level 5.00 2.00", crt_tab_object)
  crt_sendline("storm-control multicast level 5.00 2.00", crt_tab_object)
  crt_sendline("storm-control action trap", crt_tab_object)
  crt_sendline("auto qos voip trust", crt_tab_object)
  crt_sendline("ip dhcp snooping trust", crt_tab_object)
...
def port_access(model: dict, network: dict, crt_tab_object: object) -> None:
  ...
  crt_sendline("switchport port-security", crt_tab_object)
  crt_sendline("switchport port-security violation restrict", crt_tab_object)
  crt_sendline("switchport port-security aging time 30", crt_tab_object)
  crt_sendline("spanning-tree portfast", crt_tab_object)
  crt_sendline("storm-control broadcast level 5.00", crt_tab_object)
  crt_sendline("storm-control multicast level 5.00", crt_tab_object)
  crt_sendline("storm-control action trap", crt_tab_object)
  crt_sendline("storm-control action shutdown", crt_tab_object)
  crt_sendline("ip dhcp snooping limit rate 20", crt_tab_object)
  crt_sendline("spanning-tree bpduguard enable", crt_tab_object)
  crt_sendline("spanning-tree guard root", crt_tab_object)
  crt_sendline("no snmp trap link-status", crt_tab_object)
  if model["image"]["xe"] is True:
    crt_sendline("device-tracking attach-policy kba-access", crt_tab_object)
  else:
    crt_sendline("ip device tracking maximum 5", crt_tab_object)

  # voice
  if network["voice"] is not False:
    crt_sendline("switchport port-security maximum 5", crt_tab_object)
    crt_sendline("switchport port-security maximum 2 vlan access", crt_tab_object)
    crt_sendline("switchport voice vlan " + network["voice"]["id"], crt_tab_object)
    crt_sendline("auto qos voip cisco-phone", crt_tab_object)
  else:
    crt_sendline("switchport port-security maximum 2", crt_tab_object)
  ...
  if network["ise"] is True:
    ...
    if network["quarantine"] is not False:
      ...
      crt_sendline("authentication open", crt_tab_object)

    # voice
    if network["voice"] is not False:
      crt_sendline("authentication host-mode multi-domain", crt_tab_object)
      crt_sendline("authentication control-direction in", crt_tab_object)

    # misc
    crt_sendline("authentication host-mode single-host", crt_tab_object)
    crt_sendline("authentication port-control auto", crt_tab_object)
    crt_sendline("authentication order mab dot1x", crt_tab_object)
    crt_sendline("authentication priority dot1x mab", crt_tab_object)
    crt_sendline("authentication timer reauthenticate server", crt_tab_object)
    crt_sendline("authentication violation replace", crt_tab_object)
    crt_sendline("authentication event server dead action authorize", crt_tab_object)
    crt_sendline("authentication event server alive action reinitialize", crt_tab_object)
    if model["image"]["version"].startswith("15.0"):
      crt_sendline("authentication timer inactivity server", crt_tab_object)
    else:
      crt_sendline("authentication timer inactivity server dynamic", crt_tab_object)
    crt_sendline("authentication periodic", crt_tab_object)
    crt_sendline("mab", crt_tab_object)
    crt_sendline("snmp trap mac-notification change added", crt_tab_object)
    crt_sendline("snmp trap mac-notification change removed", crt_tab_object)
    crt_sendline("dot1x timeout tx-period 7", crt_tab_object)
    crt_sendline("dot1x pae authenticator", crt_tab_object)
...
def mainmenu2() -> None:
  ...
  crt_sendline("no ip proxy-arp", crt_tab_object)
  ...
  # services
  crt_sendline("service tcp-keepalives-in", crt_tab_object)
  crt_sendline("service tcp-keepalives-out", crt_tab_object)
  crt_sendline("service timestamps debug datetime localtime", crt_tab_object)
  crt_sendline("service timestamps log datetime localtime", crt_tab_object)
  crt_sendline("service password-encryption", crt_tab_object)
  crt_sendline("no service udp-small-servers", crt_tab_object)
  crt_sendline("no service tcp-small-servers", crt_tab_object)
  crt_sendline("no service pad", crt_tab_object)
  crt_sendline("no service finger", crt_tab_object)

  # misc stuff
  if model["model"].startswith("C1000"):
    crt_sendline("dying-gasp primary snmp-trap secondary syslog", crt_tab_object)
  if not model["model"].startswith("C1000"):
    crt_sendline("no ip source-route", crt_tab_object)
  crt_sendline("no ip gratuitous-arps", crt_tab_object)
  crt_sendline("no ip finger", crt_tab_object)
  crt_sendline("ip subnet-zero", crt_tab_object)
  ...
  # gui
  crt_sendline("no ip http server", crt_tab_object)
  crt_sendline("no ip http secure-server", crt_tab_object)

  # time
  crt_sendline("clock timezone CET 1", crt_tab_object)
  crt_sendline("clock summer-time CEDT recurring last Sun Mar 2:00 last Sun Oct 3:00", crt_tab_object)
  crt_sendline("ntp logging", crt_tab_object)
  for i in checks["settings"]["NTP"]["Peer"]:
    crt_sendline("ntp server " + i, crt_tab_object)
  crt_sendline("ip access-list standard ntp-peer-access", crt_tab_object)
  for i in checks["settings"]["NTP"]["Access"]:
    crt_sendline("permit " + i, crt_tab_object)
  crt_sendline("ntp access-group peer ntp-peer-access", crt_tab_object)

  # errdisable
  crt_sendline("errdisable recovery cause all", crt_tab_object)
  crt_sendline("errdisable recovery interval 300", crt_tab_object)
  crt_sendline("errdisable detect cause all", crt_tab_object)
  crt_sendline("no errdisable recovery cause bpduguard", crt_tab_object)
  crt_sendline("no errdisable recovery cause storm-control", crt_tab_object)

  # spanning-tree
  crt_sendline("spanning-tree portfast bpduguard default", crt_tab_object)
  crt_sendline("spanning-tree mode rapid-pvst", crt_tab_object)
  crt_sendline("no spanning-tree optimize bpdu transmission", crt_tab_object)
  crt_sendline("spanning-tree vlan 2-4094 priority 61440", crt_tab_object) # added 2022-09-23
  ...
  # dhcp snooping
  crt_sendline("ip dhcp snooping vlan 2-4094", crt_tab_object)
  crt_sendline("no ip dhcp snooping information option", crt_tab_object)
  crt_sendline("ip dhcp snooping", crt_tab_object)
  ...
  # ssh
  crt_sendline("ip ssh time-out 120", crt_tab_object)
  crt_sendline("ip ssh authentication-retries 5", crt_tab_object)
  crt_sendline("ip ssh version 2", crt_tab_object)
  if model["hmac"] == "sha1":
    crt_sendline("ip ssh server algorithm mac hmac-sha1", crt_tab_object)
  elif model["hmac"] == "sha2":
    crt_sendline("ip ssh server algorithm mac hmac-sha1 hmac-sha2-256 hmac-sha2-512", crt_tab_object)

  # login failure block
  crt_sendline("login block-for 60 attempts 4 within 30", crt_tab_object)
  crt_sendline("login quiet-mode access-class vty-access", crt_tab_object)
  ...
  crt_sendline("aaa authentication attempts login 2", crt_tab_object)
  crt_sendline("aaa authentication fail-message X", crt_tab_object)
  crt_sendline("Local authentication failed", crt_tab_object)
  crt_sendline("X", crt_tab_object)
  crt_sendline("aaa authentication password-prompt " + chr(34) + "Enter local password: " + chr(34), crt_tab_object)
  crt_sendline("aaa authentication username-prompt "  + chr(34) + "Enter local username: " + chr(34), crt_tab_object)
  crt_sendline("aaa authentication login local_auth local", crt_tab_object)
  crt_sendline("aaa authorization exec local_auth local", crt_tab_object)
  crt_sendline("aaa authentication login default group tac_auth local", crt_tab_object)
  crt_sendline("aaa authentication enable default none", crt_tab_object)
  crt_sendline("aaa authorization exec default group tac_auth if-authenticated", crt_tab_object)
  crt_sendline("aaa authorization commands 0 default group tac_auth if-authenticated", crt_tab_object)
  crt_sendline("aaa authorization commands 1 default group tac_auth if-authenticated", crt_tab_object)
  crt_sendline("aaa authorization commands 2 default group tac_auth if-authenticated", crt_tab_object)
  crt_sendline("aaa authorization commands 3 default group tac_auth if-authenticated", crt_tab_object)
  crt_sendline("aaa authorization commands 4 default group tac_auth if-authenticated", crt_tab_object)
  crt_sendline("aaa authorization commands 5 default group tac_auth if-authenticated", crt_tab_object)
  crt_sendline("aaa authorization commands 6 default group tac_auth if-authenticated", crt_tab_object)
  crt_sendline("aaa authorization commands 7 default group tac_auth if-authenticated", crt_tab_object)
  crt_sendline("aaa authorization commands 8 default group tac_auth if-authenticated", crt_tab_object)
  crt_sendline("aaa authorization commands 9 default group tac_auth if-authenticated", crt_tab_object)
  crt_sendline("aaa authorization commands 10 default group tac_auth if-authenticated", crt_tab_object)
  crt_sendline("aaa authorization commands 11 default group tac_auth if-authenticated", crt_tab_object)
  crt_sendline("aaa authorization commands 12 default group tac_auth if-authenticated", crt_tab_object)
  crt_sendline("aaa authorization commands 13 default group tac_auth if-authenticated", crt_tab_object)
  crt_sendline("aaa authorization commands 14 default group tac_auth if-authenticated", crt_tab_object)
  crt_sendline("aaa authorization commands 15 default group tac_auth if-authenticated", crt_tab_object)
  crt_sendline("aaa accounting commands 0 default start-stop group tac_auth", crt_tab_object)
  crt_sendline("aaa accounting commands 1 default start-stop group tac_auth", crt_tab_object)
  crt_sendline("aaa accounting commands 2 default start-stop group tac_auth", crt_tab_object)
  crt_sendline("aaa accounting commands 3 default start-stop group tac_auth", crt_tab_object)
  crt_sendline("aaa accounting commands 4 default start-stop group tac_auth", crt_tab_object)
  crt_sendline("aaa accounting commands 5 default start-stop group tac_auth", crt_tab_object)
  crt_sendline("aaa accounting commands 6 default start-stop group tac_auth", crt_tab_object)
  crt_sendline("aaa accounting commands 7 default start-stop group tac_auth", crt_tab_object)
  crt_sendline("aaa accounting commands 8 default start-stop group tac_auth", crt_tab_object)
  crt_sendline("aaa accounting commands 9 default start-stop group tac_auth", crt_tab_object)
  crt_sendline("aaa accounting commands 10 default start-stop group tac_auth", crt_tab_object)
  crt_sendline("aaa accounting commands 11 default start-stop group tac_auth", crt_tab_object)
  crt_sendline("aaa accounting commands 12 default start-stop group tac_auth", crt_tab_object)
  crt_sendline("aaa accounting commands 13 default start-stop group tac_auth", crt_tab_object)
  crt_sendline("aaa accounting commands 14 default start-stop group tac_auth", crt_tab_object)
  crt_sendline("aaa accounting commands 15 default start-stop group tac_auth", crt_tab_object)
  crt_sendline("aaa authorization config-commands", crt_tab_object)

  # line and vty
  crt_sendline("ip access-list sta vty-access", crt_tab_object)
  for i in checks["settings"]["Solarwinds"]["ipaddress"]:
    crt_sendline("permit " + i, crt_tab_object)
  for i in checks["settings"]["Management"]:
    crt_sendline("permit " + i, crt_tab_object)
  crt_sendline("line con 0", crt_tab_object)
  crt_sendline(" logging synchronous", crt_tab_object)
  crt_sendline(" login authentication local_auth", crt_tab_object)
  crt_sendline(" transport output none", crt_tab_object)
  crt_sendline(" privilege level 15", crt_tab_object)
  crt_sendline(" no password", crt_tab_object)
  crt_sendline("line vty 0 15", crt_tab_object)
  crt_sendline(" no privilege level 15", crt_tab_object)
  crt_sendline(" exec-timeout 15 0", crt_tab_object)
  crt_sendline(" logging synchronous", crt_tab_object)
  crt_sendline(" transport input ssh", crt_tab_object)
  crt_sendline(" transport output none", crt_tab_object)
  crt_sendline(" no password", crt_tab_object)
  crt_sendline(" access-class vty-access in vrf-also", crt_tab_object)
  crt_sendline(" length 40", crt_tab_object)
  crt_sendline(" width 150", crt_tab_object)

  # banner
  crt_sendline("banner motd X", crt_tab_object)
  crt_sendline(r"*", crt_tab_object)
  crt_sendline(r"* | |/ /                    | |              | |", crt_tab_object)
  crt_sendline(r"* | ' /_   _ _ __   __ _ ___| |__   __ _  ___| |", crt_tab_object)
  crt_sendline(r"* |  <| | | | '_ \ / _` / __| '_ \ / _` |/ __| |/ / _` |", crt_tab_object)
  crt_sendline(r"* | . \ |_| | | | | (_| \__ \ |_) | (_| | (__|   < (_| |", crt_tab_object)
  crt_sendline(r"* |_|\_\__,_|_| |_|\__, |___/_.__/ \__,_|\___|_|\_\__,_|", crt_tab_object)
  crt_sendline(r"* | |/ /            __/ |", crt_tab_object)
  crt_sendline(r"* | ' / ___  _ __ _|___/", crt_tab_object)
  crt_sendline(r"* |  < / _ \| '_ ` _ \| '_ ` _ \| | | | '_ " + chr(92), crt_tab_object)
  crt_sendline(r"* | . \ (_) | | | | | | | | | | | |_| | | | |", crt_tab_object)
  crt_sendline(r"* |_|\_\___/|_| |_| |_|_| |_| |_|\__,_|_| |_|", crt_tab_object)
  crt_sendline(r"*", crt_tab_object)
  crt_sendline(r"* [WARNING]", crt_tab_object)
  crt_sendline(r"* If you are not authorised to access this system", crt_tab_object)
  crt_sendline(r"* exit IMMEDIATELY.", crt_tab_object)
  crt_sendline(r"*", crt_tab_object)
  crt_sendline(r"* Unauthorised access to this system is forbidden by", crt_tab_object)
  crt_sendline(r"* national, and international laws.", crt_tab_object)
  crt_sendline(r"*", crt_tab_object)
  crt_sendline(r"* By entry into this system you acknowledge that you", crt_tab_object)
  crt_sendline(r"* are authorised to access it and have the level of", crt_tab_object)
  crt_sendline(r"* privilege at which you subsequently operate on", crt_tab_object)
  crt_sendline(r"* this system.", crt_tab_object)
  crt_sendline(r"*", crt_tab_object)
  crt_sendline(r"* You consent by entry into this system to the", crt_tab_object)
  crt_sendline(r"* monitoring of your activities.", crt_tab_object)
  crt_sendline(r"*", crt_tab_object)
  crt_sendline(r"*********************************************************", crt_tab_object)
  crt_sendline("X", crt_tab_object)

  # snmp
  crt_sendline("snmp-server contact Infra NOC", crt_tab_object)
  crt_sendline("ip access-list sta snmp-access", crt_tab_object)
  for i in checks["settings"]["Solarwinds"]["ipaddress"]:
    crt_sendline("permit " + i, crt_tab_object)
  for i in checks["settings"]["Management"]:
    crt_sendline("permit " + i, crt_tab_object)
  crt_sendline("snmp-server group " + checks["secret"]["snmp"]["username"] + " v3 priv access snmp-access", crt_tab_object)
  crt_sendline("snmp-server user " + checks["secret"]["snmp"]["username"] + " " + checks["secret"]["snmp"]["username"] + " v3 auth sha " + checks["secret"]["snmp"]["password"] + " priv aes 128 " + checks["secret"]["snmp"]["password"] + " access snmp-access", crt_tab_object)
  crt_sendline("snmp-server ifindex persist", crt_tab_object)
  crt_sendline("snmp-server contact Infra NOC", crt_tab_object)
  crt_sendline("snmp-server enable traps snmp authentication linkdown linkup coldstart warmstart", crt_tab_object)
  crt_sendline("snmp-server enable traps transceiver all", crt_tab_object)
  crt_sendline("snmp-server enable traps auth-framework", crt_tab_object)
  crt_sendline("snmp-server enable traps cpu threshold", crt_tab_object)
  crt_sendline("snmp-server enable traps storm-control trap-rate 1", crt_tab_object)
  crt_sendline("snmp-server enable traps stpx", crt_tab_object)
  crt_sendline("snmp-server enable traps port-security", crt_tab_object)
  crt_sendline("snmp-server enable traps envmon", crt_tab_object)
  crt_sendline("snmp-server enable traps mac-notification", crt_tab_object)
  crt_sendline("snmp-server enable traps errdisable", crt_tab_object)
  crt_sendline("snmp-server enable traps mac-notification change move threshold", crt_tab_object)
  crt_sendline("snmp-server enable traps errdisable", crt_tab_object)
  crt_sendline("snmp-server enable traps auth-framework sec-violation", crt_tab_object)
  for i in checks["settings"]["Solarwinds"]["ipaddress"]:
    crt_sendline("snmp-server host " + i + " version 3 priv " + checks["secret"]["snmp"]["username"], crt_tab_object)
  crt_sendline("snmp-server trap link ietf", crt_tab_object)
  if model["stack-oid"]:
    crt_sendline("no snmp-server sysobjectid type stack-oid", crt_tab_object)
  ...
  # logging
  crt_sendline("logging on", crt_tab_object)
  crt_sendline("logging buffered 16384 debugging", crt_tab_object)
  crt_sendline("logging snmp-authfail", crt_tab_object)
  crt_sendline("logging rate-limit console 10 except errors", crt_tab_object)
  crt_sendline("no logging console", crt_tab_object)
  crt_sendline("no logging message-counter syslog", crt_tab_object)
  crt_sendline("no logging origin-id", crt_tab_object)
  crt_sendline("logging monitor debug", crt_tab_object)
  if network["ise"] is False:
    for i in checks["settings"]["Solarwinds"]["ipaddress"]:
      crt_sendline("logging host " + i, crt_tab_object)

  # device-sensor
  crt_sendline("cdp run", crt_tab_object)
  crt_sendline("device-sensor filter-list cdp list cdp-list", crt_tab_object)
  crt_sendline(" tlv name device-name", crt_tab_object)
  crt_sendline(" tlv name address-type", crt_tab_object)
  crt_sendline(" tlv name capabilities-type", crt_tab_object)
  crt_sendline(" tlv name platform-type", crt_tab_object)
  crt_sendline(" tlv name version-type", crt_tab_object)
  crt_sendline("device-sensor filter-spec cdp include list cdp-list", crt_tab_object)
  crt_sendline("lldp run", crt_tab_object)
  crt_sendline("device-sensor filter-list lldp list lldp-list", crt_tab_object)
  crt_sendline(" tlv name port-id", crt_tab_object)
  crt_sendline(" tlv name port-description", crt_tab_object)
  crt_sendline(" tlv name system-name", crt_tab_object)
  crt_sendline(" tlv name system-description", crt_tab_object)
  crt_sendline(" tlv name system-capabilities", crt_tab_object)
  crt_sendline(" tlv name management-address", crt_tab_object)
  crt_sendline("device-sensor filter-spec lldp include list lldp-list", crt_tab_object)
  crt_sendline("device-sensor filter-list dhcp list dhcp-list", crt_tab_object)
  crt_sendline(" option name host-name", crt_tab_object)
  crt_sendline(" option name requested-address", crt_tab_object)
  crt_sendline(" option name parameter-request-list", crt_tab_object)
  crt_sendline(" option name class-identifier", crt_tab_object)
  crt_sendline(" option name client-identifier", crt_tab_object)
  crt_sendline("device-sensor filter-spec dhcp include list dhcp-list", crt_tab_object)
  crt_sendline("device-sensor notify all-changes", crt_tab_object)
  crt_sendline("access-session template monitor", crt_tab_object)
  if not model["image"]["version"].startswith("15.0"):
    crt_sendline("device-sensor accounting", crt_tab_object)

  # device classifier
  crt_sendline("no macro auto monitor", crt_tab_object)
  if not model["image"]["version"].startswith("15.0"):
    crt_sendline("device classifier", crt_tab_object)

  # ip device tracker
  if model["image"]["xe"] is True:
    crt_sendline("device-tracking policy kba-access", crt_tab_object)
    crt_sendline(" limit address-count 5", crt_tab_object)
    crt_sendline(" tracking enable", crt_tab_object)
    crt_sendline("device-tracking policy kba-trunk", crt_tab_object)
    crt_sendline(" trusted-port", crt_tab_object)
    crt_sendline(" device-role switch", crt_tab_object)
    crt_sendline(" no protocol ndp", crt_tab_object)
    crt_sendline(" no protocol arp", crt_tab_object)
    crt_sendline(" no protocol dhcp4", crt_tab_object)
    crt_sendline("device-tracking tracking auto-source", crt_tab_object)
  else:
    crt_sendline("ip device tracking", crt_tab_object)
    crt_sendline("ip device tracking probe delay 15", crt_tab_object)
    if model["image"]["version"].startswith("15.0"):
      crt_sendline("ip device tracking probe use-svi", crt_tab_object)
    else:
      crt_sendline("ip device tracking probe auto-source override", crt_tab_object)

  # ise specific stuff
  if network["ise"] is True:
    # mac notif
    crt_sendline("mac address-table notification change interval 30", crt_tab_object)
    crt_sendline("mac address-table notification change", crt_tab_object)
    crt_sendline("mac address-table notification mac-move", crt_tab_object)
    ...
    crt_sendline("load-balance method least-outstanding", crt_tab_object)
    crt_sendline("aaa authentication dot1x default group ise_auth", crt_tab_object)
    crt_sendline("aaa authentication dot1x ise_auth group ise_auth", crt_tab_object)
    crt_sendline("aaa authorization network default group ise_auth", crt_tab_object)
    crt_sendline("aaa authorization network ise_auth group ise_auth", crt_tab_object)
    crt_sendline("aaa authorization network auth-list group ise_auth", crt_tab_object)
    crt_sendline("aaa authorization auth-proxy default group ise_auth", crt_tab_object)
    crt_sendline("aaa accounting auth-proxy default start-stop group ise_auth", crt_tab_object)
    crt_sendline("aaa accounting dot1x default start-stop group ise_auth", crt_tab_object)
    crt_sendline("aaa accounting system default start-stop group ise_auth", crt_tab_object)
    crt_sendline("aaa accounting update newinfo periodic 15", crt_tab_object)
    crt_sendline("radius-server attribute 6 on-for-login-auth", crt_tab_object)
    crt_sendline("radius-server attribute 6 support-multiple", crt_tab_object)
    crt_sendline("radius-server attribute 8 include-in-access-req", crt_tab_object)
    crt_sendline("radius-server attribute 25 access-request include", crt_tab_object)
    crt_sendline("radius-server attribute 31 send nas-port-detail", crt_tab_object)
    crt_sendline("radius-server attribute 31 mac format ietf upper-case", crt_tab_object)
    crt_sendline("radius-server vsa send accounting", crt_tab_object)
    crt_sendline("radius-server vsa send authentication", crt_tab_object)
    crt_sendline("radius-server dead-criteria time 5 tries 3", crt_tab_object)
    crt_sendline("radius-server deadtime 2", crt_tab_object)
    crt_sendline("radius-server retry method reorder", crt_tab_object)

    # coa
    crt_sendline("aaa server radius dynamic-author", crt_tab_object)
    for i in list_psn:
      crt_sendline(" client " + i[1] + " server-key 7 " + cisco_encrypt(checks["secret"]["coa"], 7), crt_tab_object)
    crt_sendline(" auth-type all", crt_tab_object)
    crt_sendline(" ignore session-key", crt_tab_object)
    crt_sendline(" ignore server-key", crt_tab_object)
    crt_sendline("authentication critical recovery delay 1000", crt_tab_object)
    crt_sendline("authentication mac-move permit", crt_tab_object)

    # activate dot1x
    crt_sendline("dot1x system-auth-control", crt_tab_object)
    crt_sendline("dot1x critical eapol", crt_tab_object)

    # logging for ise
    crt_sendline("logging discriminator ISE facility drops AUTHMGR|EPM|DOT1X|MAB|SESSION_MGR", crt_tab_object)
    crt_sendline("logging buffered discriminator ISE", crt_tab_object)
    for server_ip in checks["settings"]["Solarwinds"]["ipaddress"]:
      crt_sendline("logging host " + server_ip + " discriminator ISE", crt_tab_object)
    #for node_fqdn in settings["ISE"]["MNT"]["Nodes"]:
    #  node_ip = dns_query(node_fqdn, checks["settings"])["ip"]
    #  crt_sendline("logging host " + node_ip + " transport udp port 20514", crt_tab_object)
...
def mainmenu13() -> None:
  ...
  crt_sendline("switchport port-security maximum " + vlaninfo[vlan["text"]]["usage"], crt_object)
  crt_sendline("switchport port-security", crt_object)
  crt_sendline("switchport port-security violation restrict", crt_object)
  crt_sendline("switchport port-security aging time 30", crt_object)
  crt_sendline("storm-control broadcast level 5.00", crt_object)
  crt_sendline("storm-control multicast level 5.00", crt_object)
  crt_sendline("storm-control action trap", crt_object)
  crt_sendline("storm-control action shutdown", crt_object)
  crt_sendline("spanning-tree portfast", crt_object)
  crt_sendline("spanning-tree bpduguard enable", crt_object)
  crt_sendline("spanning-tree bpdufilter enable", crt_object)
  crt_sendline("spanning-tree guard root", crt_object)
  crt_sendline("ip dhcp snooping limit rate 20", crt_object)
  crt_sendline("no cdp enable", crt_object)
  crt_sendline("snmp trap mac-notification change added", crt_object)
  crt_sendline("snmp trap mac-notification change removed", crt_object)
  crt_sendline("no snmp trap link-status", crt_object)
  if model["image"]["xe"] is True:
    crt_sendline("device-tracking attach-policy " + vlaninfo[vlan["text"]]["policy"], crt_object)
  else:
    crt_sendline("ip device tracking maximum " + vlaninfo[vlan["text"]]["usage"], crt_object)
  crt_sendline("no shut", crt_object)

  # ise
  if network["ise"] is True:
    crt_sendline("authentication host-mode multi-auth", crt_object)
    crt_sendline("authentication open", crt_object)
    crt_sendline("authentication port-control auto", crt_object)
    crt_sendline("authentication periodic", crt_object)
    crt_sendline("mab", crt_object)
...
try:
  ...
  print("")
  print("Cisco Provisioning Script")
  print("For Kungsbacka kommun / Municipality internal use only!")
  print("")
  ...
  while checks["status"] == "success":
    ...
    mainmenu_header = (
      ...
      " For Kungsbacka kommun / Municipality internal use only!\n"
      "=====================================\n"
      "\n"
      "Select operation:\n"
    )
    mainmenu_list = {
      "1": {
        "permission": ["se-network", "tk-broadband"],
        "space": False,
        "break": False
      },
      "2": {
      ...
      for item in mainmenu_list.items():
        if mainmenu_group in item[1]["permission"]: # type: ignore[operator]
          ...
          if "mainmenu_body" not in globals():
            ...
            mainmenu_body = indent + item[0] + "   " + globals()["mainmenu" + item[0]].__doc__.strip()
          else:
            if item[1]["space"] is True:
              # one empty row above
              mainmenu_body += "\n"
            mainmenu_body += "\n" + indent + item[0] + "   " + globals()["mainmenu" + item[0]].__doc__.strip()
      # permanent menu options
      mainmenu_body += "\n\n" + " " * ((len(max(mainmenu_list.keys(), key=len)) - 2) * 2) + "98" + "   " + "About"
      mainmenu_body += "\n" + " " * ((len(max(mainmenu_list.keys(), key=len)) - 2) * 2) + "99" + "   " + "Exit"
      mainmenu_list.update({"98": {"break": False, "permission": [mainmenu_group]}}) # add about to get though processing below
        
      if "mainmenu_body" in globals():
        # print menu and wait for input
        mainmenu_text = mainmenu_header + mainmenu_body
        mainmenu = crt_box_input(mainmenu_text, "Cisco Provisioning Script", "")
